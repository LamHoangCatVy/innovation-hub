...
function Shockwave({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (time < 1.5) {
      ...
      const progress = time / 1.5; // 0 to 1
  ...
  return (
    <mesh ref={waveRef} rotation={[Math.PI / 2, 0, 0]} visible={false}>
      <torusGeometry args={[1, 0.04, 16, 128]} />
...
function DepartmentNode({
  ...
  useFrame(({ clock }) => {
    ...
    if (time < 1.5) {
      const currentRadius = 1 + (time / 1.5) * 5;
      const dist = Math.abs(currentRadius - radius);
      if (dist < 0.6) {
        burstIntensity = (0.6 - dist) * 1.5; // Up to 0.9 extra intensity
      }
    }
    
    const mat = nodeRef.current.material as THREE.MeshStandardMaterial;
    if (mat) {
      mat.emissiveIntensity = 0.16 + burstIntensity;
    }
  });
...
export default function SupernovaOrbit() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-8 top-8 flex items-center justify-between text-xs font-semibold uppercase tracking-[0.18em] text-text-muted">
        <span className="text-amber-500">Supernova Burst</span>
