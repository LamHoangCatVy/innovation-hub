...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-40 h-screen bg-navy-900 border-r border-navy-700 flex flex-col transition-all duration-300",
        collapsed ? "w-[68px]" : "w-[260px]"
      )}
    >
      <div className="flex items-center h-16 px-4 border-b border-navy-700 gap-3">
        {!collapsed && (
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-primary-light flex items-center justify-center">
              <Lightbulb size={16} className="text-white" />
            </div>
            <span className="font-semibold text-sm text-text-primary">Yummy Innovation</span>
          </div>
        )}
        <button
          onClick={() => setCollapsed(!collapsed)}
          className={cn(
            "p-1.5 rounded-lg hover:bg-navy-800 text-text-muted transition-colors cursor-pointer",
            collapsed && "mx-auto"
          )}
        >
      ...
      <div className="p-3 border-t border-navy-700">
        <div className={cn("flex items-center gap-3 px-3 py-2", collapsed && "justify-center")}>
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-light to-gold flex items-center justify-center text-xs font-bold text-white">
            A
          </div>
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-text-primary truncate">Admin User</p>
              <p className="text-xs text-text-muted truncate">Ban Chiến lược</p>
            </div>
          )}
        </div>
      </div>
